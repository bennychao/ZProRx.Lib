...
namespace ZP.Lib
{
    ...
    public class ZTimerProperty : ZProperty<int>
    {
        ...
        public IObservable<ZNull> OnStartObservable =>
            Observable.FromEvent<Action>(h => () => h(), h => startOption += h, h => startOption -= h).Select(_ => ZNull.Default);

        public IObservable<ZNull> OnEndObservable =>
            Observable.FromEvent<Action>(h => () => h(), h => endOption += h, h => endOption -= h).Select(_ => ZNull.Default);
