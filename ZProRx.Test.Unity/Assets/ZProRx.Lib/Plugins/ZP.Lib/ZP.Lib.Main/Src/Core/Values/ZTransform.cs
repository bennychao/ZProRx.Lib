...
namespace ZP.Lib
{
    ...
    public class ZTransform
	{
        ...
        public void FaceTo(Vector2 pos)
        {
            Direction.Value = pos -  Position.Value;
        }

        public void Rotate(float a, Vector3 axis)
        {
            //Pos.Value.a = a;
#if !ZP_SERVER
            Matrix4x4 m = Matrix4x4.Rotate(Quaternion.AngleAxis(a, axis));
#else
            Matrix4x4 m = Matrix4x4.Rotate(ZTransformEx.AngleAxisEx(a, axis));
#endif
            Vector3 temp = m.MultiplyVector(new Vector3(Direction.Value.x, Direction.Value.y));
            Direction.Value = new Vector2(temp.x, temp.y);
