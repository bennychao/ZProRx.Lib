...
namespace ZP.Lib.Server.SQL
{
    public sealed partial class ZPropertySQLite
    {
        ...
        public void CreateTable<TData>(string TableName, TData sample)
        {
            ...
            sCommand.Append($"`{mainIndexStr}` INTEGER PRIMARY KEY AUTOINCREMENT,");
            ...
            {
                sCommand.Append($"`{indexStr}` INTEGER DEFAULT NULL");
            }
        ...
        public List<string> ShowDBs()
        {
            ...
            using (SqliteDataReader reader = QueryRaw(sql))
            {
                ...
                while (reader.Read())
                {
                    var dbname = reader.GetString(reader.GetOrdinal("Database"));
