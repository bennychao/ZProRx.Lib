...
{
    public  sealed partial class ZPropertySQLite
    {
        ...
        public void ConvertObject(object obj, SqliteDataReader reader)
        {
            ...
            foreach (var p in props)
            {
                try
                {
                    var index = reader.GetOrdinal(GetColumnName(p));

                    if (p.GetDefineType() == typeof(int))
                    {
                        p.Value = reader.GetInt32(index);
                    }
                    ...
                    else if (p.GetDefineType() == typeof(ZDateTime))    //before string column
                    {
                        ...
                        ((ZDateTime)p.Value).ConvertFromString(reader.GetString(index));
                    }
                    else if (p.GetDefineType() == typeof(string))
                    {
                        p.Value = reader.GetString(index);
                    }
                    else if (p.GetDefineType() == typeof(float))
                    {
                        p.Value = reader.GetFloat(index);
                    }
                    else if (p.GetDefineType() == typeof(bool))
                    {
                        p.Value = reader.GetBoolean(index);
                    }


                    else if (p.GetDefineType() == typeof(ZIntBar))
                    {
                        index = reader.GetOrdinal(GetColumnName(p) + ".Cur");
                        (p.Value as ZIntBar).Cur.Value = reader.GetInt16(index);
                    ...
                    {
                        index = reader.GetOrdinal(GetColumnName(p) + ".Cur");
                        (p.Value as ZDataBar).Cur.Value = reader.GetFloat(index);
                        //(p.Value as ZDataBar).Cur.Value = reader.GetFloat(GetColumnName(p) + ".Max");
                    }

                    else if (p.GetDefineType() == typeof(ZExp))
                    {
                        index = reader.GetOrdinal(GetColumnName(p) + ".CurRank");

                        (p.Value as ZExp).CurRank.Value = reader.GetInt16(index);

                        index = reader.GetOrdinal(GetColumnName(p) + ".CurExp");
                        (p.Value as ZExp).CurExp.Value = reader.GetFloat(index);
                    }

                    else if (p.GetDefineType().IsEnum)
                    {
                        p.Value = Enum.Parse(p.GetDefineType(), reader.GetString(index), true);
                    }
                    else if (ZPropertyMesh.IsMultiEnum(p))
                    {
                       p.Value =  (p.Value as IMultiEnumerable)?.Parse(reader.GetString(index));
                    }
                    else if (p.GetDefineType() == typeof(Vector2))
                    {
                        var v = (Vector2)p.Value;
                        index = reader.GetOrdinal(GetColumnName(p) + ".x");

                        v.x = reader.GetFloat(index);

                        index = reader.GetOrdinal(GetColumnName(p) + ".y");
                        v.y = reader.GetFloat(index);
                    }
                    else if (p.GetDefineType() == typeof(Vector3))
                    {
                        ...
                        index = reader.GetOrdinal(GetColumnName(p) + ".z");
                        v.z = reader.GetFloat(index);
                    }
                    else if (p.GetDefineType() == typeof(ZTransform))
                    {
                        var v = (ZTransform)p.Value;

                        v.Position.Value = new Vector2(reader.GetFloat(reader.GetOrdinal(GetColumnName(p) + ".Position.x")), 
                            reader.GetFloat(reader.GetOrdinal(GetColumnName(p) + ".Position.y")));
                        v.Direction.Value = new Vector2(reader.GetFloat(reader.GetOrdinal(GetColumnName(p) + ".Direction.x")), 
                            reader.GetFloat(reader.GetOrdinal(GetColumnName(p) + ".Direction.y")));

                    }
                    else if (ZPropertyMesh.IsRef(p))
                    {
                        (p as IRefable).RefID = (int)reader.GetInt32(reader.GetOrdinal(GetColumnName(p) + ".refID"));
                    }
                    else
                    {
                        if (p.AttributeNode.IsDefined<DBJsonStringAttribute>())
                        {
                            ...
                            ZPropertyPrefs.LoadValueFromStr(p, reader.GetString(index));
                        }
                    }
                }
                catch (Exception e)
