...
namespace ZP.Lib
{
    ...
    public static class ZPropertyObservable
    {
        ...
        static public IObservable<ZNull> OnAction(Action action)
        {
            return Observable.FromEvent(h => action += h, h => action -= h).Select(_=>ZNull.Default);
        }

        static public IObservable<ZNull> OnEventObservable(this IZEvent zEvent)
        {
            return Observable.FromEvent(h => zEvent.OnEvent += h, h => zEvent.OnEvent -= h).Select(_=>ZNull.Default);
        }
