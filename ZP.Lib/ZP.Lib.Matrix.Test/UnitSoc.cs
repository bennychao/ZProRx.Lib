...
namespace ZP.Lib.Matrix.Test
{
    ...
    public class UintSoc
    {
        internal class StartUp
        {
            ...
            public void Configure(ISocAppBuilder app)
            {
                ...
                var clientBuilder = app.UseRoomClient()
                    ...
                    ////! [For Dudu Chapter]
                    //.AddPipeline<DuduBattlePipeline>()
        ...
        static async Task TestClientAsync(IRoomClient client)
        {
            ...
            channelClient.Post<TestRet>("testfunc_with_rawdata_no_return", retObj).Subscribe(ret =>
            {
                taskEnd.Value = true;
            });
            ...
            channelClient.Post<TestRet>("testfunc_with_no_return", retObj).Subscribe(ret =>
            {
                taskEnd.Value = true;
            });
            ...
            //call testfunc_void
            channelClient.Post<ZNull>("testfunc_void", ZNull.Default).Subscribe(ret =>
            {
                taskEnd.Value = true;
            });
