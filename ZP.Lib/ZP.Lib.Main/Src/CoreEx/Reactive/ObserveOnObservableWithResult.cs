...
#if ZP_SERVER
    //not used
    internal class ObserveOnObservableWithResult<T> : OperatorObservableBase<T>
    {
        readonly IObservable<T> source;
        readonly IScheduler scheduler;

        public ObserveOnObservableWithResult(IObservable<T> source, IScheduler scheduler)
        ...
        class ObserveOn : OperatorObserverBase<T, T>
        {
            ...
            readonly ObserveOnObservableWithResult<T> parent;
            readonly LinkedList<SchedulableAction> actions = new LinkedList<SchedulableAction>();
            bool isDisposed;

            public ObserveOn(ObserveOnObservableWithResult<T> parent, IObserver<T> observer, IDisposable cancel) : base(observer, cancel)
            {
                this.parent = parent;
            }
        ...
        class ObserveOn_ : OperatorObserverBase<T, T>
        {
            ...
            public ObserveOn_(ObserveOnObservableWithResult<T> parent, ISchedulerQueueing scheduler, IObserver<T> observer, IDisposable cancel) : base(observer, cancel)
            {
                this.parent = parent;
                this.scheduler = scheduler;
