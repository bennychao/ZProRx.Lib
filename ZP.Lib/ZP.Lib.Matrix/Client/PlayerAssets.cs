...
namespace ZP.Lib.Matrix.Client
{
    public class PlayerAssets : PropObjectSingleton<PlayerAssets>
    {
        ...
        public async Task<(ZNetErrorEnum error, TCardLink link)> ProduceCardAsync<TCard, TCardLink>(uint cardId)
            ...
            where TCardLink : BaseCardLink<TCard>
        {
            ...
            var error = await DoConsumesAsync(consumes);
        ...
        async Task<ZNetErrorEnum> DoConsumesAsync(ICansumable consume)
        {
            //check cards
        ...
        async Task<ZNetErrorEnum> DoRecyclesAsync(uint pid, CardRecycle recycle)
        {
