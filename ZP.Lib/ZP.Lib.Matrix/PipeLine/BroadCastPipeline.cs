...
namespace ZP.Lib.Matrix
{
    ...
    {
        public IObservable<Unit> BroadCast<T>(string action, T data = default(T))
        {
        ...
        public IObservable<Unit> BroadCastMsg<T>(T data)
        {
            return BroadCast<T>("msg_raw", data);
        }

        public new IObservable<Unit> Post<T>(string action, T data = default(T))
        {
            var url = GetBaseUrl() + "_broadcast_/" + action;
