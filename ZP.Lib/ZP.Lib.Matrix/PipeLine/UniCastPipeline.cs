...
namespace ZP.Lib.Matrix
{
    ...
    public class UniCastPipeline : BaseCastPipeline, IChannelClient, IUniCast
    {
        ...
        public new  IObservable<Unit> Post<T>(string action, T data)
        {
            if (CurClientId == "")
        ...
        public IObservable<Unit> UniCast<T>(string curClientId, string action, T data)
        {
            if (curClientId == "")
                throw new Exception("no support curClientId id");
        ...
        public IObservable<Unit> UniCastMsg<T>(string curClientId, T data)
        {
